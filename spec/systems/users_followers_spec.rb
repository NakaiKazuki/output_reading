...
RSpec.describe 'UsersFollowers', type: :system do
  ...
  describe 'users/:id/followers layout' do
    it 'ログインしていなくても閲覧可能' do
      ...
      expect(page).to have_current_path followers_user_path(user), ignore_query: true
    end
    ...
    describe 'User　Profile' do
      ...
      describe 'ログインしているユーザーが一致しない場合' do
        ...
        it 'フォローとフォロー解除ボタンが機能している', js: true do
          ...
          expect(page).to have_current_path followers_user_path(other_user), ignore_query: true
          expect {
            click_button('フォロー解除')
            wait_for_ajax
          }.to change { other_user.followers.count }.by(-1)
    ...
    describe 'Show_Follow' do
      before { create(:relationship, follower: other_user, followed: user) }
